...
export function createPlayerExportData(players: Player[]) {
	const data = players.map(p => {
		...
		const pointsAsKicker =
			(scoring?.points ?? 0) + (p.scoring_stats?.kicker?.points ?? 0);
		const pointsPerGame = appearances
			? +(pointsAsKicker / appearances).toFixed(2)
			: 0;
...
export function createAppearanceExportData(
	appearances: (PlayerAppearanceStats & Player)[],
) {
