        ...
        >
          ...
          <Column header="Club">
            <template #body="slotProps">
              <img :src="`src/assets/${slotProps.data.nrl_club}.svg`" :alt="slotProps.data.nrl_club" :height="50" :width="50" />
            </template>
          </Column>
          <Column field="player_name" header="Name" :sortable="true"></Column>
          <Column header="Positions" :sortable="true">
            <template #body="slotProps">
              <div>{{ slotProps.data.position
            }}{{ slotProps.data.position2 ? `, ${slotProps.data.position2}` : '' }}</div>
            </template>
          </Column>
          <Column
            field="stats.appearances"
            header="Appearances"
            :sortable="true"
          ></Column>
          <Column
            field="times_as_captain"
            header="Captained"
            :sortable="true"
          ></Column>
        </DataTable>
