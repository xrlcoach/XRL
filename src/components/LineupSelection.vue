<template>
  <div id="lineupContainer" :style="powerplay ? `border-radius: 5px; border: 1px solid var(--pink-500); box-shadow: 0px 0px 20px var(--pink-500);` : ''">
...
<script lang="ts">
  ...
  import { Player, PlayerLineupEntry, XrlFixture, XrlUser } from '../global';
  ...
  import { PositionMap, PositionNumbers } from '../services/utils';
  ...
  export default defineComponent({
    setup() {
      ...
      const existingLineup = ref([] as PlayerLineupEntry[]);
      ...
      onMounted(async () => {
        ...
        const nextRound = await GetNextRoundNotInProgress();
        if (nextRound) {
          ...
          let opponentTeamShort = homeGame
            ? fixture.value?.away
            : fixture.value?.home;
          let opponentInfo = await GetUserInfoByTeamShort(
            opponentTeamShort || ''
          );
        ...
        existingLineup.value = await GetUserLineup();
        for (let position of Object.keys(selections.value.starters)) {
          selections.value.starters[
            position as keyof typeof selections.value.starters
          ] =
